...
export async function seedFredrickAnderson() {
  ...
  if (existing.length > 0) {
    ...
    if (fred.balance !== "18276999.30") {
      await db.update(customers).set({ balance: "18276999.30" }).where(eq(customers.id, fred.id));
      console.log("[seed] Fixed Fredrick Anderson balance to $18,276,999.30");
    }
    const existingTransfers = await db.select().from(transfers).where(eq(transfers.customerId, fred.id));
    if (existingTransfers.length === 0) {
      await seedFredrickTransfers(fred.id);
      console.log("[seed] Added 19 transfers for Fredrick Anderson");
    }
    console.log("[seed] Fredrick Anderson already exists, skipping seed.");
    return;
  }

  console.log("[seed] Creating Fredrick Anderson account...");

  const hashedPassword = await bcrypt.hash("Fred0817@@@", 10);
  ...
  const [fred] = await db.insert(customers).values({
    ...
    email: null,
    phone: null,
    address: "7795 Sendero Angelica",
    city: "San Diego",
    state: "CA",
    zipCode: "92127",
    country: "United States",
    gender: "male",
    accountNumber: "7943218605",
  ...
  await seedFredrickTransfers(fredId);

  console.log(`[seed] Fredrick Anderson created successfully (ID: ${fredId}) with 22 transactions, 19 transfers, and access code 394827`);
}

async function seedFredrickTransfers(customerId: number) {
  const transferData = [
